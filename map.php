...
function hook($url, $reject = '', int $behave = 0): string // throws \InvalidArgumentException
{// receive a request: extract and validate the path to navigate
    ...
    for ($i = 0, $n = \strlen($url); $i < $n; ++$i) {               // MUST validate percent-escapes syntactically (no decode)
        if ($url[$i] === '%') {
            ...
            || !\ctype_xdigit($url[$i + 2]))                        || throw new \InvalidArgumentException('hook:invalid pct-encoding');
            $i += 2;
        }
    }

    $path = \trim($url, '/');
    ($reject === '' || !isset($path[\strcspn($path, $reject)]))     || throw new \InvalidArgumentException('hook:forbidden chars');
    return $path;
...
{// direct lookup: does a file exist at this exact location? or in that nested location (with REBASE) ?
    ...
    if(\is_file($leaf)){                                            // cheap stat guards against expensive realpath on miss
        $real = \realpath($leaf);
        if($real !== false && \strpos($real, $base) === 0)          // canonical is within base boundary
            $file = $real;
    }
