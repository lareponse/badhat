...
{// execute callable inside a single atomic transaction
    ...
    try {                                                           // outer try/catch to manage transaction atomicity
        ...
        try {
            ...
            throw _ErrorInfoException($pdo, 'transaction callable', 400, $t);
        }
        $pdo->commit()                                              || throw _ErrorInfoException($pdo, 'commit', 500);
    } catch (\Throwable $t) {
        $pdo->inTransaction() && ($pdo->rollBack()                  || throw _ErrorInfoException($pdo, 'rollback', 500, $t));
