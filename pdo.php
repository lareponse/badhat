...
{// query/prepare/process (params=[], then prepare-only)
    ...
    if ($params === null) return $pdo->query($query)                ?: throw new \RuntimeException(ERRMODE_SILENT($pdo, 'query'));
    $stm = $pdo->prepare($query, $prep_options)                     ?: throw new \RuntimeException(ERRMODE_SILENT($pdo, 'prepare'));
    if ($params) $stm->execute($params)                             || throw new \RuntimeException(ERRMODE_SILENT($stm, 'execute'));
    return $stm;
...
{// execute callable inside a single atomic transaction
    ...
    try {                                                           // outer try/catch to manage transaction atomicity
        $pdo->beginTransaction()                                    || throw new \RuntimeException(ERRMODE_SILENT($pdo, 'beginTransaction'));
        try {
            $res = $transaction($pdo);
        } catch (\Throwable $t) {
            throw new \RuntimeException(ERRMODE_SILENT($pdo, 'callable', $t), 0xBADC0DE);
        }
        $pdo->commit()                                              || throw new \RuntimeException(ERRMODE_SILENT($pdo, 'commit'));
    } catch (\Throwable $t) {
        $pdo->inTransaction() && ($pdo->rollBack()                  || throw new \RuntimeException(ERRMODE_SILENT($pdo, 'rollback', $t)));
        throw $t;                                                   // rethrow original error after rollback
    }
    return $res;
}// return the callable result after successful commit

function ERRMODE_SILENT(\PDO|\PDOStatement $source, string $action, ?\Throwable $t = null): string
{
